...
class AppProvider extends ChangeNotifier {
  ...
  List<StudySessionModel> get recentSessions {
    ...
    return _sessions
        .where((s) {
          if (seen.contains(s.subjectId)) return false;
    ...
    try {
      ...
      if (subjectsRemote is List) {
        _subjects = subjectsRemote
            .map((e) =>
                SubjectModel.fromJson(Map<String, dynamic>.from(e as Map)))
            .toList();
      }
      ...
      if (sessionsRemote is List) {
        _sessions = sessionsRemote
            .map((e) =>
                StudySessionModel.fromJson(Map<String, dynamic>.from(e as Map)))
            .toList();
      }
      ...
      if (groupsRemote is List) {
        _groups = groupsRemote
            .map(
                (e) => GroupModel.fromJson(Map<String, dynamic>.from(e as Map)))
            .toList();
      }
  ...
  Future<void> upsertSession(StudySessionModel session) async {
    final idx = _sessions.indexWhere((s) => s.id == session.id);

    int durationDiff = session.durationMinutes;
    bool isNewSession = idx < 0;

    // 1. Add or update the session in the list
    if (isNewSession) {
      _sessions.insert(0, session);
    } else {
      // If updating, calculate only the NEW minutes added since the last save
      durationDiff = session.durationMinutes - _sessions[idx].durationMinutes;
      _sessions[idx] = session;
    }

    // 2. Add the accumulated time to the specific Subject
    if (durationDiff > 0) {
      final subIdx = _subjects.indexWhere((s) => s.id == session.subjectId);
      if (subIdx != -1) {
        _subjects[subIdx].totalMinutes += durationDiff;
        await _saveSubjects();
      }
    }

    // 3. Update User Stats (Total Time, Sessions Done, and Streaks)
    if (session.isQualifying) {
      _user.totalMinutesStudied += durationDiff;

      // Only increment streak and "Total Sessions" if this is the first time this session is saved
      if (isNewSession) {
        _user.totalSessions++;
        final today = _dateOnly(DateTime.now());
